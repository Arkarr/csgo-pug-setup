...
By default the plugin uses the ``cfg/sourcemod/pugsetup/live.cfg`` config when going live. You are free to change this file all you want (or change which file is used via the ``sm_pugsetup_live_cfg`` cvar). (Note: if you use knife rounds, make sure ``mp_give_player_c4 1`` is in this file!)
...
- ``sm_pugsetup_maplist_aim_maps`` (default "aim_maps.txt") - If using ``sm_pugsetup_use_aim_map_warmup 1``, Maplist file in addons/sourcemod/configs/pugsetup to use. You may also use a workshop collection ID instead of a maplist you if you have the [SteamWorks](https://forums.alliedmods.net/showthread.php?t=229556) extension installed.
...
- ``sm_pugsetup_team_size`` (default 5) - Maximum size of a team when selecting teams.
- ``sm_pugsetup_time_format`` (default "%Y-%m-%d_%H") - Time format to use when creating demo file names.
- ``sm_pugsetup_use_aim_map_warmup`` (default 0) - Whether to change map to a random map from sm_pugsetup_maplist_aim_maps during warmup periods. Note that the plugin will ``exec sourcemod/pugsetup/aim_warmup.cfg`` when on an aim_ map.
